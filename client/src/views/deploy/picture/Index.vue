  ...
  v-container
    v-layout(row wrap)
      v-flex(mr-2 xs12 sm6 md2)
        v-select(height="1" :items="listPos" label="类型" outline item-text="name" item-value="id" clearable v-model="list.type")
      v-flex(mr-2 xs12 sm6 md2)
      ...
      v-card
        ...
        v-card-text
          v-form(ref="form" v-model="valid" lazy-validation)
            v-text-field(v-model="ruleForm.name" :rules="[v => !!v || 'Name is required']" label="图片名称" required)
            v-select(v-model="ruleForm.d_id" :items="listDep" item-text="name" item-value="id" :rules="[v => !!v || 'Department is required']" label="类型")
